    ...
    public class TwilioSmsService : ISmsMessagingService
    {
        ...
        public async Task<SendSingleSmsRequestResult> SendSingleSmsAsync(string to, string subject, string body, string userId = "", string companyId = "")
        {
            ...
            var message = MessageResource.Create(
                body: $"{subject}\n\n{body}",
                from: new PhoneNumber(_messagingConfiguration.TwilioConfiguation.From),
        ...
        public Task<SendVerificationSmsRequestResult> SendVerificationSmsAsync(string to)
        {
            TwilioClient.Init(_messagingConfiguration.TwilioConfiguation.AccountSID, _messagingConfiguration.TwilioConfiguation.AuthToken);
            ...
            return Task.FromResult(new SendVerificationSmsRequestResult() {
                Status = verification.Status == "pending" ? "pending" : verification.Status == "approved" ? "approved" : "deffered",
                Sid = verification.Sid
            });
        }

        public Task<VerifySmsTokenRequestResult> VerifySmsTokenAsync(string to, string code)
        {
            TwilioClient.Init(_messagingConfiguration.TwilioConfiguation.AccountSID, _messagingConfiguration.TwilioConfiguation.AuthToken);
            ...
            return Task.FromResult(new VerifySmsTokenRequestResult()
            {
                Status = verificationCheck.Status == "approved" ? "approved" : "deffered",
